...
base64EncodedRequestSignature request =
    Signature.signBase64Unsafe "/home/owainlewis/.oraclebmc/bmcs_api_key.pem"
...
addAuthHeader request keyId =
    let headers = (BS.intercalate " ") . map (original . fst) . requestHeaders in
    do
        signature <- base64EncodedRequestSignature request
        let requestSignature = zipHeaderPairs [ ("version", "1")
                                              , ("headers", headers request)
                                              , ("keyId", keyId)
                                              , ("algorithm", "rsa-sha256")
                                              , ("signature", signature)
                                              ]
        return $ setRequestHeader "Authorization"
...
transformRequest credentials request =
    let keyId = Credentials.keyId credentials in
      addGenericHeaders request >>=
        (flip addAuthHeader keyId)
