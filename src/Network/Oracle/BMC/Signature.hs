...
-- | Simple transform from an RSAKeyPair to a concrete PrivateKey type.
...
signPure input = do
    key <- loadPrivateKey ("/home/owainlewis/.oraclebmc/bmcs_api_key.pem")
    return $ C.toStrict <$> Pure.sign key (C.fromStrict input)

signBase64  :: C8.ByteString -> IO (Either Pure.RSAError C8.ByteString)
signBase64 input = do
    signature <- signPure input
